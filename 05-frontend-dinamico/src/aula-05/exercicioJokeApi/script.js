const jokeTitle = document.querySelector(".joke-setup")
const jokeDelivery = document.querySelector(".joke-delivery")

const nextJokeButton = document.querySelector("#next-joke-button")
const favoriteJokeButton = document.querySelector("#favorite-joke-button")

const favoriteJokesContainer = document.querySelector(".favorites-jokes-container")
...
async function displayMainJoke() {
  const dataJoke = await fetchJoke()

  jokeTitle.innerHTML = dataJoke.setup
  jokeDelivery.innerHTML = dataJoke.delivery

  favoriteJokeButton.onclick = () => addFavoriteJoke(dataJoke)
}

function addFavoriteJoke(dataJoke) {
  const jokes = getFavoriteJokes()

  const hasCurrentJoke = jokes.some(joke => joke.id === dataJoke.id)

  if (!hasCurrentJoke) {
    const newFavoriteJoke = {
      id: dataJoke.id,
      setup: dataJoke.setup,
      delivery: dataJoke.delivery,
    }

    setFavoriteJokes([...jokes, newFavoriteJoke])

    displayFavoriteJokes()
  }
}

function setFavoriteJokes(jokes) {
...
function displayFavoriteJokes() {
  ...
  jokes.length === 0 
    ? favoriteJokesContainer.classList.add("hidden") 
    : favoriteJokesContainer.classList.remove("hidden")

  favoriteJokesList.innerHTML = ""
  jokes.map(joke => {
    const jokeItemList = document.createElement("li")
    jokeItemList.classList.add("card")
    jokeItemList.innerHTML = `
      <div class="joke-content">
        <h2 class="joke-setup">${joke.setup}</h2>

        <p class="joke-delivery">${joke.delivery}</p>
      </div>
    `
    const removeFromFavoritesButton = document.createElement("button")
    removeFromFavoritesButton.classList.add("button", "button-transparent")
    ...
    removeFromFavoritesButton.addEventListener("click", () => removeFromFavorites(joke.id))

    jokeItemList.append(removeFromFavoritesButton)

    favoriteJokesList.append(jokeItemList)
  })
}
...
function displayLoading() {
  jokeTitle.innerHTML = "Carregando piada..."
...
nextJokeButton.addEventListener("click", displayMainJoke)
